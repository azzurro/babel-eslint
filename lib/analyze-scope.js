...
const changeParentInChain = (node, parent, scope) => {
    if (Array.isArray(node)) {
        node.forEach(x => changeParentInChain(x, parent, scope));
        return;
    }

    const type = node && node.type;

    if (!(type in t || type === 'Property'))
        return;

    for (const value of Object.values(node)) {
        if (node !== value && node !== parent)
            changeParentInChain(value, node, scope);
    }
    ...
    if (type == 'JSXIdentifier') {
        ...
        if (variableScope.set.has(name)) {
            const iden = t.identifier(name);
            iden.parent = parent;

            variableScope.__referencing(iden);
        }
    }
...
class Referencer extends OriginalReferencer {
  ...
  visitFunction(node) {
    // Decorators.
    changeParentInChain(node.decorators, node, this.currentScope());
